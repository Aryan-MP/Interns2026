...
# ===== AZURE CONFIGURATION =====
ENDPOINT = "https://smgaivision.cognitiveservices.azure.com/"
# NOTE: do NOT commit real keys. Use environment variables or Azure Key Vault.
KEY = os.environ.get("AZURE_COMPUTERVISION_KEY", "<your-key-here>")
...
def main_menu():
    """Interactive menu for the practical"""
    while True:
        print("\n" + "█"*70)
        print("█  AZURE COMPUTER VISION + CAMERA INTEGRATION")
        print("█"*70)
        print("\nOptions:")
        print("  1. Capture image from camera & analyze")
        print("  2. Analyze existing image from file")
        print("  3. View captured images")
        print("  4. Exit")
        print("-"*70)
        ...
        if choice == '1':
            ...
            if image_path:
                analysis = analyze_local_image(image_path)
                if analysis:
                    save_results_to_file(image_path, analysis)
            ...
            if os.path.exists(file_path):
                analysis = analyze_local_image(file_path)
                if analysis:
                    save_results_to_file(file_path, analysis)
            else:
            ...
            print("\n👋 Exiting Azure Vision Practical. Goodbye!")
            break
...
if __name__ == "__main__":
    print("\n" + "█"*70)
    print("█  WELCOME TO AZURE COMPUTER VISION PRACTICAL")
    print("█"*70)
    print("\nInitializing Azure Vision Client...")
    print(f"  ✓ Endpoint: {ENDPOINT}")
    print(f"  ✓ Region: East US")
    print(f"  ✓ API Type: Computer Vision")
    print("\nTesting connection to Azure...")
    
    try:
        # Test connection
        print("  ✓ Azure Vision API connected successfully!")
        print("\n✓ All systems ready. Starting application...\n")
        time.sleep(1)
        main_menu()
    
    except Exception as e:
        print(f"\n❌ Failed to connect to Azure Vision API:")
        print(f"   {str(e)}")
        print("\nPlease check:")
